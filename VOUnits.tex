...
\previousversion{\footnotesize{\url{http://www.ivoa.net/internal/IVOA/UnitsDesc/WD-VOUnits-v1.0-20120820.pdf}}}
...
%\hbox to \textwidth{\hfil\tiny \SVNHeadURL}
...
By systematically comparing the four reference schemes described above (details are given in Appendix~\ref{sec:comparisons}), 
...
case-sensitive strings of chars consisting of printable ASCII characters} (values coded 20 to 7E in hexadecimal).
...
The VOUnits base symbols are the following : 
\medskip
...
prefixes to any base symbol (except that~P is not allowed as a scale
factor for \unit{a}). 

These scale factors are written for VOUnits as in Table 5 of the FITS 2010 paper : 
...
The SI prefixes of \prettyref{tab:comparUnitScale} always refer to
...
Mibi, and so on) of \cite[\S4]{std:iec80000-13}.
...
The VOUnits adopted symbols are the following :
...
following the legacy practice, and in order to avoid the (unlikely)
...
A few symbols which might be ambiguous are listed in \prettyref{tab:ambiguous}, with their correct 
...
symbols for \aa{}ngstr\"om, barn, erg and gauss, as written below :
%in the sixth column of \prettyref{tab:comparUnitDeprecated}.
...
\prettyref{tab:comparUnitOther} compares other various symbols. 

The last set of VOUnits symbols, derived from this comparison, is the following :
...
A summary of the various symbols is given in the last part of Appendix~\ref{sec:comparisons}.
...
\section{Comparison of existing schemes and VOUnits\label{sec:comparisons}}
...
In the table below, we indicate the `known units' for each of the
...
\makeatother
\catcode`\%=11
\tablehead{\emph{unit}&\emph{fits}&\emph{ogip}&\emph{cds}&\emph{vou}\\}
\begin{supertabular}{rcccc}
...
\prettyref{tabx:vougrammar}.  This grammar is a strict subset of the
