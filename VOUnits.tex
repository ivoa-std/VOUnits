...
In general, this specification is concerned almost exclusively with
...
should instead just make this information available.
...
A \textbf{symbol} within a unit-component should be parsed as follows:
\begin{enumerate}
\item If it corresponds to a known \textbf{base symbol}, then it
should be recognised as such -- the \unit{Pa} is always the Pascal,
and the \texttt{P} is not parsed as a `peta' prefix (this is
effectively a special case which is distinguished from the `normal'
case below).
...
syntaxes), base symbols may be put between single
quotes \unit{'...'}.  Such symbols are always parsed as
...
should not be interpreted as a tenth of a byte.\footnote{We have no
...
interpreting \units{dB/cd} as 0.9\units{mbit/s}.}  The year can be expressed by
...
\subsection{Preceding numerical factor}

A VOUnits unit may be preceded by a numerical scale-factor to indicate a
derived unit.  For example, the inch might appear as the unit
of \unit{25.4mm}.  See \prettyref{appx:vougrammar} for the syntax of
the VOUnits numerical string.
...
one might label it as having units of \unit{1.898E27kg}.
The \emph{advantage} of doing so is that the data consumer can
translate the column data into physical units without further
information, and the data source is thus self-contained.
The \emph{disadvantage} of doing so is (i) that the intention might be
obscured; and (ii) that the measurements may be relative to the actual
...
\subsection{Quoted, unknown, units\label{sec:quoting}}

In the VOUnits syntax (a significant divergence from the other
syntaxes), base symbols may be put between single
quotes \unit{'...'}.  Such symbols are always parsed as
unrecognised unit symbols which are not further examined.
...
\subsection{Deviations between VOUnits and other grammars}
...
leading floats, and why that may not be a good idea

unrecognised units are OK, and why one might or should not use them

Are seconds always SI/atomic seconds?
...
Avoiding the use of special characters (\AA, ', $\mu$,\dots) reduces the
...
\clearpage
\section{History: Comparison of unit-string schemes\label{appx:comparisons}}
...
between syntaxes, and so imply the motivation for this Recommendation,
namely that the current practice is disturbingly heterogeneous.
...
\begin{quotation}
\texttt{0\textbackslash.[0-9]+([eE][+-]?[0-9]+)?} or
\texttt{[1-9][0-9]*(\textbackslash.[0-9]+)?([eE][+-]?[0-9]+)?}
\end{quotation}
(that is, something resembling \texttt{1.5e+11}).
