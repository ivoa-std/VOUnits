...
\urlthisversion{\footnotesize{\url{http://www.ivoa.net/Documents/VOUnits/20120731/}}}
...
Catalogues, Version 2.0 \citep{cds00}. They are presented in the fourth column
of the comparison tables.
...
A formal grammar summarizing these conventions is given in \prettyref{sec:grammar}.
...
and a century corresponds to \unit{ha} or \unit{hyr}. Note
  ...
  stipulation~\cite{si-brochure}.\footnote{If large telescope arrays 
...
Scale factors from \prettyref{tab:comparUnitScale} should however be preferred and used whenever possible.
...
The rules defined in \prettyref{sec:proposal} allow to build VOUnit parsers.
...
the symbols and notations defined in the standard for astronomical catalogues \citep{cds00}, as well as
...
The formal grammar used for VOUnits is given in \prettyref{sec:grammar}.
...
\prettyref{tab:terminals}.% on p.\pageref{tab:terminals}.
\begin{table}
...
\texttt{SIGNED\_INTEGER}&an integer with a non-optional leading sign\raggedright\tabularnewline
...
\end{tabular}
\caption{\label{tab:terminals}The terminals used in the grammars}
\end{table}
...
associated tables.  Our preferred FITS grammar is in \prettyref{tab:fitsgrammar} on p.\pageref{tab:fitsgrammar}.

\begin{table}
\verbatiminput{unity-grammars/unity-fits.txt}
\caption{\label{tab:fitsgrammar}The FITS grammar}
\end{table}
...
grammar is in \prettyref{tab:ogipgrammar} on p.\pageref{tab:ogipgrammar}.

\begin{table}
\verbatiminput{unity-grammars/unity-ogip.txt}
\caption{\label{tab:ogipgrammar}The OGIP grammar}
\end{table}
...
For the CDS units syntax, see \cite{cds00}.  Our preferred CDS
grammar is in \prettyref{tab:cdsgrammar} on p.\pageref{tab:cdsgrammar}.

\begin{table}
\verbatiminput{unity-grammars/unity-cds.txt}
\caption{\label{tab:cdsgrammar}The CDS grammar}
\end{table}
...
\prettyref{tab:vougrammar} on p.\pageref{tab:vougrammar}.  This grammar is a strict subset of the
...
\caption{\label{tab:vougrammar}The VOUnits grammar}
\end{table}
...
\begin{itemize}
\item 1.0-20120718 to 1.0-20120731
  \begin{itemize}
